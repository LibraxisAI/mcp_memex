...
impl RAGPipeline {
    ...
    pub async fn search(&self, query: &str, k: usize) -> Result<Vec<SearchResult>> {
        ...
        let candidates = self
            .storage
            .search_store(query_embedding.clone(), k * 3)
            .await?;
        ...
        if !candidates.is_empty() {
            let documents: Vec<String> = candidates
                ...
                .collect();
            let metadatas: Vec<serde_json::Value> = candidates
                .iter()
    ...
    async fn extract_text(&self, path: &Path) -> Result<String> {
        ...
        if ext == "pdf" {
            ...
            let pdf_text = tokio::task::spawn_blocking(move || pdf_extract::extract_text(&path))
                .await??;
            return Ok(pdf_text);
        }
